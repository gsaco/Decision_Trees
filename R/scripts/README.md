...
- Categorical variables were converted to factors for the classification model in R
...
- Alpha (complexity parameter or cp in rpart) controls tree pruning
...
- Successfully created a randomized treatment variable using rbinom (50% probability)
...
- Random variation (noise using rnorm)

### OLS Estimation
Two regression models were fitted using lm():
...
The Random Forest model (using randomForest package) estimates individual-level treatment effects (CATE):
...
The representative decision tree (using rpart with maxdepth=2) shows:
...
### Feature Importance
Feature importance analysis using randomForest reveals:
...
### Tercile Analysis
The heatmap (using ggplot2) of standardized covariates by treatment effect terciles shows:
...
- Decision trees (rpart) provide an interpretable method for predicting heart disease
...
### R-Specific Notes
- Used rpart for decision trees with cp (complexity parameter) for pruning
- randomForest package for causal effect estimation
